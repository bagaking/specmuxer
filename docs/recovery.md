...
2. Run `specmuxer status --json` to inspect persisted sessions under `.specmuxer/`.
...
4. Verify health with `specmuxer top` and inspect logs (`specmuxer logs --session <id> --tail 40`).

## Resuming Specific Sessions

```
specmuxer resume --session sess-123 --session sess-456
```
